    ...
    for filename in files:
        with open(os.path.join(directory, filename), 'r') as file:
            ...
            try:
                ...
                # BERT 캡션과 GPT-2 캡션을 초기화
                bert_caption = None
                gpt2_caption = None

                for line in lines:
                    if line.startswith("BERT Caption:"):
                        bert_caption = preprocess_caption(line.split(':')[1].strip())
                    elif line.startswith("GPT-2 Caption:"):
                        gpt2_caption = preprocess_caption(line.split(':')[1].strip())
                
                # 이미지 ID를 키로 사용하여 캡션 저장
...
# 성능 지표 계산
bert_metrics, gpt2_metrics = calculate_individual_performance_metrics(after_caption, label_data)
